"""Start SeeNot at login: two LaunchAgents, the Kev model server and the app.

    seenot-desktop install      write and load them (both start now, and at every login)
    seenot-desktop uninstall    stop and remove them

launchd restarts either one if it crashes. Logs go to ~/Library/Logs/SeeNot/.
...
LOGS = Path.home() / "Library" / "Logs" / "SeeNot"
KEV_LABEL, APP_LABEL = "com.seenot.kev", "com.seenot.app"
...
def install(repo: Path, kev_dir: Path, model: str = "jaredpalmer/kev-4b", port: int = 8009) -> None:
    ...
    app = _plist(APP_LABEL, [uv, "run", "seenot-desktop", "app"], repo,
    ...
    print("Grant them in System Settings > Privacy & Security, then: seenot-desktop install  (again, to restart).")
