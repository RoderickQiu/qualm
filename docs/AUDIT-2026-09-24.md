...
- **Your Mac's memory.** It was deep in swap all night (25 of 26 GB of swap used at one point).
  Every process the agents started competed with your apps. Your own Qualm's readings slowed
...
2. In the checkout: `uv sync`, then start it again the way you did before:
   ...
   ```bash
   cd ~/Documents/qualm && nohup uv run qualm app >> ~/Library/Logs/Qualm/app.log 2>&1 &
   ```
...
- Workflow scripts and every agent's full transcript:
  `~/.claude/projects/-Users-r-Documents-qualm/ddde46e4-fc71-4fd4-afc8-5cff043f77a9/`
  (`workflows/scripts/` and `subagents/workflows/`).
